...
size_t llama_state_get_size(const struct llama_context *ctx)
{
    ...
    const size_t s_total = (+s_rng_size + s_rng + s_n_outputs + s_output_pos + s_logits_size + s_logits + s_embedding_size + s_embedding + s_kv_buf_size + s_kv_head + s_kv_size + s_kv_used + s_kv + s_kv_cells);
...
static void llama_state_get_data_internal(struct llama_context *ctx, llama_data_context *data_ctx)
{
    // copy rng
    {
        ...
        rng_ss.setf(std::ios::dec | std::ios::left);
        fprintf(stderr, "Setting locale using std::locale::classic() with dec | left formatting\n");
        rng_ss << ctx->rng;
...
size_t llama_state_set_data(struct llama_context *ctx, const uint8_t *src)
{
    ...
    // set rng
    {
        ...
        rng_ss.imbue(std::locale::classic());
        fprintf(stderr, "Loading ctx->rng from string stream\n");
        rng_ss >> ctx->rng;
